...
// DOM Templating classes v21-dev © copyright (cc-by) Kroc Camen 2012-2023
...
/*  Basic API:
    ...
    appendAfter( query, content )
            append content after an element (i.e. add siblings)
    remove( query )
            remove one or more HTML elements, attributes or classes
    replaceText( query, search, replace )
            search & replace text-nodes within elements
    replaceTextArray( query, strings )
            as above but use an array of ['search' => 'replace'] strings
    repeat( query )
...
// return a table of HTML entities:
//==============================================================================
// because everything is XML, HTML named entities like "&copy;" will cause
// blank (error) output. we need to convert these named entities back to
// real UTF-8 characters (which XML doesn’t mind)
//
function htmlEntities (): array {
    // a table of HTML entities to reverse. static so that once built,
    // we do not build it again (slow)
    ...
    //
    if (is_null( $html_entities )) {
        // '&', '<', '>' are removed so we don’t
        // turn user text into working HTML!
        $html_entities = array_flip( array_diff(
...
{
    protected \DOMNode  $DOMNode;       // reference to source DOMNode
    private   \DOMXPath $DOMXPath;      // an internal XPath object, so you
                                        //  don't have to manage one externally
    protected array     $namespaces;    // optional XML namespaces

    // create_fragment : given plain-text, turn it into a small DOM tree
    //--------------------------------------------------------------------------
    private function create_fragment (
        string  $text
    ) : \DOMDocumentFragment {
        $frag = $this->DOMNode->ownerDocument->createDocumentFragment();
        $frag->appendXML(
            // appendXML must always use XML rather than HTML
            self::toXML( $text )
        );
        return $frag;
    }
    ...
    public function toXML (
        ...
        //
        // TODO: isn't this going to alter <pre> content if not escaped?
        while (preg_match(
            '/<(?!!)                            # exclude DOCTYPE|CDATA
                [a-z-]+\s+                      # HTML tag name
                (?:                             # iterate attributes...
                    [a-z-]+=                    # ignore ones with values
                    (?:                         # either...
                        [a-z-]+                 # value without quotes
                    |   (["\']) [^\1]* \1       # or value with quotes
                    )\s+
                )*                              # allow for multiple attrs
                ([a-z-]+)                       # capture HTML-style attr
                (?=\s|>)                        # (end of attribute|HTML tag)
            /xis',
    ...
    public function query (
        ...
        $xpath = implode( '|', array_map(
            // note PHP8.1's `callable` syntax here to reference the method
            // without actually calling it, and doing away with strings
            // TODO: use a `foreach` instead of `array_map`?
    ...
    public function append (
        ...
        foreach ($this->query( $query ) as $node){
            // convert the plain-text to an XML fragment
    ...
    // appendAfter: append content after another element
    //--------------------------------------------------------------------------
    public function appendAfter (
        string $query,          // node query to select reference element(s)
        string $content         // content to append
    ) {
        // execute query, loop returned node(s) and
        // attach the HTML/XML fragment to the node
        foreach ($this->query( $query ) as $node){
            ...
            // check if the node is the last sibling
            if ($node->parentNode->lastChild === $node){
                // for the last sibling, append a new child of the parent
                $node->parentNode->appendChild( $frag );
            } else {
                // if there's some kind of HTML after the reference node,
                // we can use that to insert our item in between
    ...
    // search and replace text within text nodes
    //--------------------------------------------------------------------------
    public function replaceText (
        string  $query,         // node query to select elements to search
        string  $search,        // the string to find, case-sensitive
        string  $replace        // the replacement string to insert
    ) {
        foreach (
            // execute xpath query, loop returned node(s)
            $this->query( $query ) as $node
        ) foreach (
            // select all text nodes within this element (and children)
            $this->DOMXPath->query( ".//text()", $node ) as $node
        ){
            // search for presence of string in text node
            $pos = mb_strpos( $node->nodeValue, $search );
            // if found, swap the text
            if ($pos !== false) $node->replaceData(
                $pos, mb_strlen( $search ), $replace
            );
        }
        return $this;
    }

    // search and replace text within text nodes using an array of searches
    //--------------------------------------------------------------------------
    public function replaceTextArray (
        string  $query,         // node query to select elements to search
        array   $strings        // ['search' => 'replace'] array of strings
    ) {
        foreach (
            // execute xpath query, loop returned node(s)
            $this->query( $query ) as $node
            ...
            // iterate each search => replace pair
            $strings as $search => $replace
        ){
            // search for presence of string in text node
            // TODO: I assume the DOMCharacterData API is using Unicode?
            $pos = mb_strpos( $node->nodeValue, $search );
            // if found, swap the text
            if ($pos !== false) $node->replaceData(
                $pos, mb_strlen( $search ), $replace
    ...
    public function __toString (): string {
        ...
        $source = ($this->doctype == TYPE_HTML)
        ?   $this->DOMNode->ownerDocument->saveHTML(
            ...
            ($this->DOMNode == $this->DOMNode->ownerDocument->documentElement)
            ? NULL : $this->DOMNode
        ...
        // [2] convert XML-style attributes (`<a attr="attr">`) to HTML-style
        ...
        while (preg_match(
            '/<(?!!)                            # exclude DOCTYPE|CDATA
                [a-z-]+\s+                      # XML tag name
                (?:                             # iterate attributes...
                    [a-z-]+\s+                  # ignore ones without values
                )*                              # allow for multiple attrs
                (([a-z-]+)                      # capture XML-style attr
                    =(["\'])? \2 \3             # quotes, value must = name!
                )(?=\s|>)                       # (end of attribute|XML tag)
            /xis',
            $source, $m, PREG_OFFSET_CAPTURE
        ))  $source = substr_replace(
            // replace the full attribute `attr="attr"` with the short version
            // `attr`. replacing starts at the beginning of the attribute.
            // note that we are replacing a longer string with a shorter one
            // so the length given is that of the full attribute so that the
            // remaining characters are not left behind!
            //
            $source, $m[2][0], $m[2][1], strlen ($m[1][0])
        );

        // [3] strip out CDATA markers
...
//==============================================================================
class DOMTemplate
    extends DOMTemplateNode
{
    // internal reference to the PHP `DOMDocument` for the template's HTML
    private $DOMDocument;

    // what type of data are we processing?
    protected $doctype = TYPE_HTML;

    // new DOMTemplate : instantiation
    //--------------------------------------------------------------------------
    public function __construct (
        // a string of the HTML/XML to form the template
        string  $source,
        ...
        $this->doctype
            = substr_compare ($source, '<?xml', 0, 4, true) === 0
            ? TYPE_XML : TYPE_HTML
        ;

        // load the template string to work with. it _must_ have
        // only one root (wrapping) element; e.g. `<html>`
        $this->DOMDocument = new \DOMDocument();
        //libxml_use_internal_errors( true );
        if ($this->doctype === TYPE_HTML) {
            if (!$this->DOMDocument->loadHTML(
                // <https://stackoverflow.com/a/20528324>
                "\xEF\xBB\xBF".str_replace("\r", '', $source),
            ...
            |   LIBXML_HTML_NOIMPLIED   // do not automatically add DOCTYPE
            |   LIBXML_HTML_NODEFDTD    // do not automatically add DTD
            |   LIBXML_NOWARNING        // (disable warnings and errors
            |   LIBXML_NOERROR          //  written to output by libxml)
            )) trigger_error(
                "Source is invalid HTML", E_USER_ERROR
            );
        } else {
            if (!$this->DOMDocument->loadXML(
                // <https://stackoverflow.com/a/20528324>
                "\xEF\xBB\xBF".str_replace("\r", '', self::toXML( $source )),
                // <https://www.php.net/manual/en/libxml.constants.php>
                LIBXML_COMPACT          // libxml >= 2.6.21
            |   LIBXML_NONET            // do not connect to external resources
            |   LIBXML_HTML_NOIMPLIED   // do not automatically add DOCTYPE
...
class DOMTemplateRepeaterArray
{
    ...
    // refer to `DOMTemplateNode->replaceText`
    public function replaceText (
        string  $query,         // node query to select elements to search
        string  $search,        // the string to find, case-sensitive
        string  $replace        // the replacement string to insert
        ...
        foreach ($this->nodes as $node)
            $node->replaceText( $query, $search, $replace )
        ;
        return $this;
    }

    // refer to `DOMTemplateNode->replaceTextArray`
    public function replaceTextArray (
        string  $query,         // node query to select elements to search
        array   $strings        // ['search' => 'replace'] array of strings
    ) {
        foreach ($this->nodes as $node)
            $node->replaceTextArray( $query, $strings )
        ;
        return $this;
    }
}
    ...
    extends DOMTemplateNode
{
    ...
    public function next (): self {
        ...
        $this->refNode = (
            ...
            // use that to insert our item in between. this means that the list
